...
import com.highwayns.payment.config.KafkaSseConsumer;
import com.highwayns.payment.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/payment-kafka")
public class PaymentKafkaResource {

    private final Logger log = LoggerFactory.getLogger(PaymentKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public PaymentKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
